...
import { ERC20Abi } from "../../global/models/ERC20Abi";
import { strongServiceV1Abi } from "../models/strongServiceV1Abi";
import { strongServiceV2Abi } from "../models/strongServiceV2Abi";
...
export const getStrongBalanceOfWallet = async (walletAddress: string) => {
  try {
    const balanceInWei: BigNumber = await strongTokenContract.balanceOf(walletAddress);
    const balance: number = Number.parseFloat(
      ethers.utils.formatUnits(balanceInWei, "wei")
    );
    return balance;
  } catch (error) {
    console.error(error);
...
export const getServiceV1UnclaimedRewards = async (walletAddress: string) => {
  try {
    ...
    const unclaimedRewardsFromV1 = Number.parseFloat(
      ethers.utils.formatUnits(unclaimedRewardsFromV1InWei, "wei")
    );
...
export const getServiceV2UnclaimedRewards = async (walletAddress: string) => {
  try {
    const entityNodeCount: number =
      await strongServiceV2Contract.entityNodeCount(walletAddress);
    let unclaimedRewardsFromV2InWei = 0;
    for (let nodeId = 1; nodeId <= entityNodeCount; nodeId++) {
      const rewardOfNodeIdInWei: number =
        await strongServiceV2Contract.getNodeReward(walletAddress, nodeId);
      unclaimedRewardsFromV2InWei += rewardOfNodeIdInWei;
    }
    const unclaimedRewardsFromV2 = Number.parseFloat(
      ethers.utils.formatUnits(unclaimedRewardsFromV2InWei, "wei")
    );
...
export const getServiceV1NumOfNodes = async (walletAddress: string) => {
  try {
    const entityNodeCount: number = await strongServiceV1Contract.entityNodeCount(
      walletAddress
    );
    return entityNodeCount;
  } catch (error) {
    console.error(error);
    return 0;
  }
